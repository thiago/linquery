...
export class ModelRegistry {
  ...
  /**
   * Registers a model class in the system. Ensures that the model is not already registered
   * before adding it to the collection of models.
   *
   * @param {ModelClass<T>} modelClass - The class of the model to register, which extends BaseModel.
   * @return {void} - Does not return any value.
   * @throws {Error} - Throws an error if the model is already registered.
   */
  register<T extends BaseModel>(modelClass: ModelClass<T>) {
    ...
    for (const model of this.getAll()) {
      ...
      for (const [fieldName, field] of Object.entries(fields)) {
        if (field.type === "relation") {
          ...
          if (!relatedModel) {
            throw new Error(
              `Model '${model.name}' has a relation field '${fieldName}' referencing unknown model '${(field as any).model}'`
            )
          }
